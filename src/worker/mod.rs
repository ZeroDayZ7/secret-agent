...
/// Okno wyprzedzenia, w jakim sekret jest traktowany jako "zbliżający się
/// do wygaśnięcia" i kwalifikuje się do odnowienia przed faktycznym TTL.
const RENEWAL_LOOKAHEAD: Duration = Duration::from_secs(30);

/// Interwał okresowego sprawdzania cache pod kątem sekretów do odnowienia.
...
pub async fn run_renewal_loop(cache: Arc<SecretCache>, kms_client: KmsClient, config: AgentConfig) {
    ...
    loop {
        tokio::select! {
            _ = poll_timer.tick() => {
                let expiring = cache.keys_expiring_within(RENEWAL_LOOKAHEAD);
                ...
                match refresh_all(&cache, &kms_client, &config).await {
                    ...
                    Err(err) => {
                        tracing::warn!(error = %err, backoff_ms = backoff.as_millis(), "odnowienie sekretów nie powiodło się, ponawiam z backoff");
